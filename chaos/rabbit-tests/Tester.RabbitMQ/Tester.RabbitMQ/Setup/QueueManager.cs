...
namespace Tester.RabbitMQ.Setup
{
    public class QueueManager
    {
        ...
        public async Task MeetDesiredStateAsync(string resourceGroup, string queuePrefix, int queueCount, string exchange, string outputQueue)
        {
            await PrepareQueuesRabbitMqAsync(queuePrefix, queueCount, exchange, outputQueue);
            await this.zkHelper.PrepareResourceGroupAsync(resourceGroup, queuePrefix, queueCount);
        }

        private async Task PrepareQueuesRabbitMqAsync(string queuePrefix, int queueCount, string exchange, string outputQueue)
        {
            await PutQueueRabbitMQAsync(outputQueue, nodes[currentNode]);
            ...
            else if (queueNames.Count < queueCount)
            {
                ...
                for (int i = 0; i < addCount; i++)
                {
                    ...
                    if (currentNode == this.nodes.Count)
                        currentNode = 0;
                    await PutBoundQueueRabbitMQAsync(exchange, queue, node);
        ...
        private async Task PutBoundQueueRabbitMQAsync(string exchange, string queueName, string node, string vhost = "%2f")
        {
            if (vhost == "/")
                vhost = "%2f";
            ...
            if (!createExchangeResponse.IsSuccessStatusCode)
                throw new Exception($"Failed to create exchange {exchange}, response {createExchangeResponse.StatusCode}");
            ...
            if (!createQueueResponse.IsSuccessStatusCode)
                throw new Exception($"Failed to create queue {queueName}, response {createQueueResponse.StatusCode}");
            ...
            if (!createBindingsResponse.IsSuccessStatusCode)
                throw new Exception($"Failed to create exchange to queue bindings, response {createBindingsResponse.StatusCode}");
        }
        
        private async Task PutQueueRabbitMQAsync(string queueName, string node, string vhost = "%2f")
        {
            if (vhost == "/")
                vhost = "%2f";
